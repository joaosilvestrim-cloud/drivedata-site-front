-- ===========================================================================
-- Vagas iniciais da DriveData (6 oportunidades do nosso nicho).
-- Rode no SQL Editor do Supabase do SITE, DEPOIS da migration 006_vagas.sql.
--
-- Todas entram como RASCUNHO (status = 'draft'), ou seja, NAO aparecem no site.
-- Revise cada uma no admin (Carreiras -> Vagas), principalmente:
--   - beneficios (escrevi o que costuma ser padrao; confirme o que a DriveData
--     realmente oferece);
--   - local e modelo de contrato (CLT x PJ);
--   - senioridade e faixa de experiencia.
-- Depois de revisar, publique pelo botao "Publicar" no admin, ou rode:
...
Vale-refeicao ou vale-alimentacao flexivel
Orcamento anual para cursos e certificacoes
Horario flexivel e modelo hibrido
...
Diferencial: Microsoft Fabric, Tabular Editor ou DAX Studio',
 'Plano de saude e odontologico
Vale-refeicao ou vale-alimentacao flexivel
Trabalho remoto com apoio para o setup de casa
Orcamento anual para cursos e certificacoes Microsoft
...
Trabalho remoto com horario flexivel
Projetos com operacoes de grande porte
...
Possibilidade de efetivacao',
 'draft', 'pt')

on conflict (slug) do nothing;

-- Conferencia:
-- select slug, title, area, work_model, contract_type, status from job order by created_at;
